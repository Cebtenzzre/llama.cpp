        ...
        for (int il = 0; il < n_layer; ++il) {
            struct ggml_tensor * tmp =
                    ggml_rope_custom_inplace(ctx0,
                        ...
                        K_shift, n_embd_head, 0, 0, n_orig_ctx, freq_base, freq_scale,
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // self-attention
        {
            ...
            struct ggml_tensor * Kcur = ggml_rope_custom(
                ctx0, ggml_reshape_3d(ctx0, tmpk, n_embd_head, n_head_kv, n_tokens),
                KQ_pos, n_embd_head, 0, 0, n_orig_ctx, freq_base, freq_scale,
                ext_factor, attn_factor, beta_fast, beta_slow
            );
            ...
            struct ggml_tensor * Qcur = ggml_rope_custom(
                ctx0, ggml_reshape_3d(ctx0, tmpq, n_embd_head, n_head, n_tokens),
                KQ_pos, n_embd_head, 0, 0, n_orig_ctx, freq_base, freq_scale,
                ext_factor, attn_factor, beta_fast, beta_slow
            );
...
static struct ggml_cgraph * llm_build_falcon(
            ...
            struct ggml_tensor * tmp =
                    ggml_rope_custom_inplace(ctx0,
                        ggml_view_3d(ctx0, kv_self.k,
                            ...
                            ggml_element_size(kv_self.k)*n_embd_gqa,
                            ggml_element_size(kv_self.k)*n_embd_gqa*n_ctx*il),
                        K_shift, n_embd_head, 2, 0, n_orig_ctx, freq_base, freq_scale,
                        ext_factor, attn_factor, beta_fast, beta_slow);
            offload_func_kq(tmp);
            ggml_build_forward_expand(gf, tmp);
        }
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // TODO: refactor into common function (shared with LLaMA)
        {
            ...
            struct ggml_tensor * Qcur = ggml_rope_custom(
                ctx0, tmpq, KQ_pos, n_embd_head, 2, 0, n_orig_ctx,
                freq_base, freq_scale, ext_factor, attn_factor, beta_fast, beta_slow
            );
            offload_func_kq(Qcur);
            struct ggml_tensor * Kcur = ggml_rope_custom(
                ctx0, tmpk, KQ_pos, n_embd_head, 2, 0, n_orig_ctx,
                freq_base, freq_scale, ext_factor, attn_factor, beta_fast, beta_slow
            );
            offload_func_kq(Kcur);
        ...
        for (int il = 0; il < n_layer; ++il) {
            struct ggml_tensor * tmp =
                    ...
                    ggml_rope_custom_inplace(ctx0,
                        ...
                        K_shift, n_rot, 2, 0, n_orig_ctx, freq_base, freq_scale,
                        ext_factor, attn_factor, beta_fast, beta_slow
                    );
            offload_func_kq(tmp);
            ggml_build_forward_expand(gf, tmp);
    ...
    for (int il=0; il < n_layer; ++il) {
        ...
        // self attention
        {
            ...
            struct ggml_tensor * qrotated =  ggml_rope_custom(
                ctx0, qrot, KQ_pos, n_rot, 2, 0, n_orig_ctx, freq_base, freq_scale,
                ext_factor, attn_factor, beta_fast, beta_slow
            );
            offload_func_kq(qrotated);
            struct ggml_tensor * krotated = ggml_rope_custom(
                ctx0, krot, KQ_pos, n_rot, 2, 0, n_orig_ctx, freq_base, freq_scale,
                ext_factor, attn_factor, beta_fast, beta_slow
            );
